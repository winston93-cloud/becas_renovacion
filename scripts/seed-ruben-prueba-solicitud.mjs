/**
 * Crea RUBEN PRUEBA PRUEBA (primaria) para probar solicitud nueva.
 * Uso: node --env-file=.env.local scripts/seed-ruben-prueba-solicitud.mjs
 */
import { createAdminClient } from '@insforge/sdk';

const REF = 29903;
const CLAVE = 'admin123';
const EMAIL = 'isc.escobedo@gmail.com';

function cicloCalendario() {
  const now = new Date();
  const year = now.getFullYear();
  const month = now.getMonth() + 1;
  const day = now.getDate();
  const startYear =
    month > 7 || (month === 7 && day >= 10) ? year : year - 1;
  return startYear - 2003;
}

async function main() {
  const admin = createAdminClient({
    baseUrl: process.env.INSFORGE_URL,
    apiKey: process.env.INSFORGE_API_KEY,
  });
  const db = admin.database;
  const ahora = new Date().toISOString();

  let { data: alumno } = await db
    .from('alumno')
    .select('alumno_id, alumno_ref')
    .eq('alumno_ref', REF)
    .maybeSingle();

  if (!alumno) {
    const { data: maxRow } = await db
      .from('alumno')
      .select('alumno_id')
      .order('alumno_id', { ascending: false })
      .limit(1)
      .maybeSingle();
    const nextId = Number(maxRow?.alumno_id || 9000) + 100;
    const { data: created, error } = await db
      .from('alumno')
      .insert([
        {
          alumno_id: nextId,
          alumno_ref: REF,
          alumno_app: 'RUBEN',
          alumno_apm: 'PRUEBA',
          alumno_nombre: 'PRUEBA',
          alumno_nivel: 3,
          alumno_grado: 3,
          alumno_grupo: 1,
          alumno_status: 1,
          alumno_nuevo_ingreso: 0,
          alumno_ciclo_escolar: cicloCalendario(),
          alumno_boleta: 0,
          mes: 0,
          secret_key: `prueba-ruben-${Date.now()}`,
          motivo: 'PRUEBA',
          responsable: 'SISTEMAS',
          digito: 0,
          alumno_permiso_solicitud_beca: 0,
          alumno_solicitud_acceso_enviada: 0,
        },
      ])
      .select('alumno_id, alumno_ref')
      .single();
    if (error) throw new Error(`crear alumno: ${error.message}`);
    alumno = created;
  } else {
    await db
      .from('alumno')
      .update({
        alumno_app: 'RUBEN',
        alumno_apm: 'PRUEBA',
        alumno_nombre: 'PRUEBA',
        alumno_nivel: 3,
        alumno_grado: 3,
        alumno_grupo: 1,
        alumno_status: 1,
        alumno_permiso_solicitud_beca: 0,
        alumno_solicitud_acceso_enviada: 0,
        alumno_solicitud_acceso_en: null,
      })
      .eq('alumno_id', alumno.alumno_id);
  }

  const aid = Number(alumno.alumno_id);

  // Sin beca del ciclo pasado → puede Solicitud nueva
  await db.from('alumno_beca').delete().eq('alumno_id', aid);

  const { data: det } = await db
    .from('alumno_detalles')
    .select('detalle_id')
    .eq('alumno_id', aid)
    .maybeSingle();
  if (det?.detalle_id) {
    await db
      .from('alumno_detalles')
      .update({ alumno_clave: CLAVE })
      .eq('detalle_id', det.detalle_id);
  } else {
    const { data: maxDet } = await db
      .from('alumno_detalles')
      .select('detalle_id')
      .order('detalle_id', { ascending: false })
      .limit(1)
      .maybeSingle();
    const nextDet = Number(maxDet?.detalle_id || 10000) + 50;
    const { error } = await db.from('alumno_detalles').insert([
      { detalle_id: nextDet, alumno_id: aid, alumno_clave: CLAVE },
    ]);
    if (error) throw new Error(`detalles: ${error.message}`);
  }

  for (const tutor_id of [1, 2]) {
    const { data: fam } = await db
      .from('alumno_familiar')
      .select('familiar_id')
      .eq('alumno_id', aid)
      .eq('tutor_id', tutor_id)
      .maybeSingle();
    const row = {
      alumno_id: aid,
      tutor_id,
      familiar_app: tutor_id === 1 ? 'MAMA' : 'PAPA',
      familiar_apm: 'PRUEBA',
      familiar_nombre: 'PRUEBA',
      familiar_email: EMAIL,
      familiar_recibir_email: 1,
      familiar_vive: 1,
      familiar_factura: 0,
      familiar_actualizacion: ahora,
    };
    if (fam?.familiar_id) {
      await db.from('alumno_familiar').update(row).eq('familiar_id', fam.familiar_id);
    } else {
      let lastErr = null;
      for (let attempt = 0; attempt < 8; attempt++) {
        const { error } = await db.from('alumno_familiar').insert([row]);
        if (!error) {
          lastErr = null;
          break;
        }
        lastErr = error;
        const m = String(error.message || '').match(
          /null value in column "([^"]+)"/
        );
        if (!m) break;
        const col = m[1];
        if (col.endsWith('_en') || col.includes('fecha') || col.includes('registro')) {
          row[col] = ahora;
        } else if (col.includes('email')) {
          row[col] = EMAIL;
        } else {
          row[col] = 0;
        }
      }
      if (lastErr) throw new Error(`familiar ${tutor_id}: ${lastErr.message}`);
    }
  }

  // Limpiar solicitudes previas de prueba
  const { data: sols } = await db
    .from('becas_solicitud')
    .select('id')
    .eq('alumno_id', aid);
  for (const s of sols || []) {
    await db.from('becas_solicitud_documento').delete().eq('solicitud_id', s.id);
    await db.from('becas_solicitud').delete().eq('id', s.id);
  }

  console.log(
    JSON.stringify(
      {
        ok: true,
        alumno: 'RUBEN PRUEBA PRUEBA',
        alumno_ref: REF,
        clave: CLAVE,
        nivel: 'Primaria 3 / A',
        tramite: 'solicitud nueva',
        docs_esperados: 5,
      },
      null,
      2
    )
  );
}

main().catch((e) => {
  console.error(e);
  process.exit(1);
});
