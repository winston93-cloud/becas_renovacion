/**
 * Crea LAURA PRUEBA PRUEBA (primaria) para probar solicitud nueva (primera vez).
 * Estado: sin permiso aún → puede pedir acceso; sin beca del ciclo pasado; sin solicitud previa.
 *
 * Uso: node --env-file=.env.local scripts/seed-laura-prueba-solicitud.mjs
 */
...
async function main() {
  ...
  if (!alumno) {
      ...
      .insert([
        {
          ...
          alumno_boleta: 0,
          mes: 0,
          secret_key: `prueba-laura-${Date.now()}`,
          motivo: 'PRUEBA',
    ...
    await db
      ...
      .update({
        ...
        alumno_permiso_solicitud_beca: 0,
        alumno_solicitud_acceso_enviada: 0,
        alumno_solicitud_acceso_en: null,
      })
  ...
  // Sin beca del ciclo pasado → flujo Solicitud nueva
  ...
  if (det?.detalle_id) {
    ...
    if (error) throw new Error(`detalles: ${error.message}`);
  }
  ...
  for (const s of sols || []) {
    ...
    await db.from('becas_solicitud_hermano').delete().eq('solicitud_id', s.id);
  ...
  console.log(
    JSON.stringify(
      {
        ok: true,
        alumno: 'LAURA PRUEBA PRUEBA',
        alumno_ref: REF,
        clave: CLAVE,
        nivel: 'Primaria 3 / A',
        tramite: 'solicitud nueva (primera vez)',
        estado: 'puede_solicitar_acceso',
